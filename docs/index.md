...
### NextGen FHIR API (Referring Provider EHR Data Import — 50%+ Ophthalmology Market Share)
- [PRD: NextGen FHIR API PMS Integration](experiments/49-PRD-NextGenFHIRAPI-PMS-Integration.md) — Inbound clinical data import from referring ophthalmology practices via NextGen's FHIR R4 Patient Access API (Patient, Condition, MedicationRequest, AllergyIntolerance, Encounter, Observation) with SMART on FHIR auth, clinical reconciliation engine, C-CDA parsing, and Mirth Connect bridge for HL7v2 practices
...
### RingCentral API (Unified Patient Communications — Voice, SMS, Fax, Video, Transcription & Analytics)
- [PRD: RingCentral API PMS Integration](experiments/71-PRD-RingCentralAPI-PMS-Integration.md) — RingCentral REST API integration for unified patient communications with OAuth 2.0 JWT authentication, SMS appointment reminders, click-to-call from patient records, call recording linked to encounters, fax-based referral workflows, telehealth video visits, webhook-driven inbound event processing, Audio AI auto-transcription with speaker diarization and encounter note generation, Interaction Analytics sentiment/emotion detection, Business Analytics call volume/SMS delivery/no-show correlation dashboards, and HIPAA-compliant BAA-covered voice/SMS/fax/video
- [RingCentral API Setup Guide](experiments/71-RingCentralAPI-PMS-Developer-Setup-Guide.md) — Developer Console app creation, JWT credential generation, RingCentralClient Python SDK wrapper with Audio AI and Business Analytics methods, CommsService FastAPI router, CallerIDResolver, ReminderScheduler, TranscriptionService with async webhook callback, CallAnalyticsService with call volume/SMS/sentiment/no-show stats, ngrok webhook tunnel, Next.js Communications Dashboard with TranscriptViewer and CommsAnalyticsDashboard
- [RingCentral API Developer Tutorial](experiments/71-RingCentralAPI-Developer-Tutorial.md) — Hands-on onboarding: build an appointment reminder pipeline with PHI-free SMS templates, patient confirmation processing via inbound webhook, click-to-call with automatic logging, call recording transcription with speaker diarization, encounter note generation, sentiment analysis, and communication analytics dashboard end-to-end
...
### Xero API (Cloud Accounting, Invoicing, Payment Reconciliation & Company Backup)
- [PRD: Xero API PMS Integration](experiments/75-PRD-XeroAPI-PMS-Integration.md) — Xero cloud accounting API integration for automated invoice generation from PMS encounters, payment tracking, bank reconciliation, financial reporting (P&L, Balance Sheet, Aged Receivables), and on-demand full company backup with encrypted local storage — completing the revenue cycle as the financial backbone connecting pVerify eligibility (Exp 73), FrontRunnerHC discovery (Exp 74), and Availity claims (Exp 47) with PHI de-identification boundary (no HIPAA BAA)
- [Xero API Setup Guide](experiments/75-XeroAPI-PMS-Developer-Setup-Guide.md) — OAuth 2.0 authorization flow, XeroClient with rate limiting (60/min), PHI scrubber, PostgreSQL schema (invoices, payments, contacts, backups, audit log), InvoiceService, XeroBackupService with full/incremental export and AES-256-GCM encryption, FastAPI endpoints, and BackupManager React component
- [Xero API Developer Tutorial](experiments/75-XeroAPI-Developer-Tutorial.md) — Hands-on onboarding: build an encounter-to-invoice pipeline with PHI de-identification, contact creation, invoice generation, payment recording, full company backup with integrity verification, and incremental backup scheduling end-to-end

### Redis (In-Memory Cache, Message Broker & Task Queue)
- [PRD: Redis PMS Integration](experiments/76-PRD-Redis-PMS-Integration.md) — Redis 8 in-memory data store integration for sub-millisecond session caching, API response caching with automatic invalidation, real-time clinical notifications via Pub/Sub and Streams, distributed sliding-window rate limiting, background task queuing via arq for report generation and batch operations, and feature flag evaluation — serving as the horizontal infrastructure layer accelerating WebSocket (Exp 37), pVerify (Exp 73), FrontRunnerHC (Exp 74), Xero (Exp 75), and RingCentral (Exp 71) integrations
- [Redis Setup Guide](experiments/76-Redis-PMS-Developer-Setup-Guide.md) — Docker Compose redis:8-alpine deployment, redis.conf with password auth and hybrid persistence, RedisClient async singleton with connection pooling, SessionService, CacheMiddleware with HIT/MISS headers, RateLimitService with sorted sets, NotificationService Pub/Sub, arq background workers, node-redis 5.11 for Next.js, and admin health/stats endpoints
- [Redis Developer Tutorial](experiments/76-Redis-Developer-Tutorial.md) — Hands-on onboarding: build a patient lookup cache-aside pipeline with sub-millisecond reads, real-time Pub/Sub notifications for encounter updates and lab results, sliding window rate limiting for login protection, and background task queuing end-to-end

### Mirth Connect (Healthcare Integration Engine — HL7v2, FHIR, X12 EDI Message Routing)
- [PRD: Mirth Connect PMS Integration](experiments/77-PRD-MirthConnect-PMS-Integration.md) — Mirth Connect 4.5.2 healthcare integration engine for HL7v2 ADT/ORM/ORU message routing, FHIR R4 gateway, lab results ingestion, X12 EDI claims translation, and real-time channel monitoring — serving as the interoperability hub connecting PMS to external EHRs, labs, pharmacies, and payers via standardized healthcare messaging protocols
...
### OptiFreight Logistics (Healthcare Shipping & Freight Management)
- [PRD: OptiFreight PMS Integration](experiments/79-PRD-OptiFreight-PMS-Integration.md) — Cardinal Health OptiFreight® Logistics integration for automated prescription delivery shipping, lab specimen routing, and supply procurement with multi-carrier rate optimization, TotalVue™ analytics, EDI X12 transactions (856/810/850), real-time tracking via webhooks, Redis-cached shipment status, HIPAA-compliant PHI minimization, and complementary positioning alongside FedEx (Exp 64), UPS (Exp 65), and OnTime 360 (Exp 66) carrier integrations
- [OptiFreight Setup Guide](experiments/79-OptiFreight-PMS-Developer-Setup-Guide.md) — Account provisioning with Cardinal Health, OptiFreightClient async Python wrapper with OAuth2 auth, SQLAlchemy shipment/event models, FastAPI shipping router with rate quotes and label generation, Redis tracking cache, webhook receiver with HMAC signature verification, Next.js ShippingDashboard with track/ship/rates tabs, and PrescriptionTracker progress bar component
...
### AutoResearch (Autonomous ML Model Optimization — AI Agent-Driven Experiment Loops)
- [PRD: AutoResearch PMS Integration](experiments/84-PRD-AutoResearch-PMS-Integration.md) — Karpathy's autonomous ML experimentation framework for overnight model optimization of PMS clinical AI models (dermatology CDS, MedASR), with agent-driven hypothesis generation, 5-minute time-boxed training runs, git-native experiment tracking, human-in-the-loop model promotion gate, and HIPAA-compliant PHI isolation
- [AutoResearch Setup Guide](experiments/84-AutoResearch-PMS-Developer-Setup-Guide.md) — Docker deployment with NVIDIA GPU passthrough, AutoResearch container configuration, PostgreSQL experiment history schema, FastAPI run management endpoints, Next.js experiment dashboard with real-time metrics, and model promotion approval workflow
- [AutoResearch Developer Tutorial](experiments/84-AutoResearch-Developer-Tutorial.md) — Hands-on onboarding: write a research program (program.md) for dermatology CDS optimization, adapt train.py for vision models, start autonomous experiment runs, monitor val_bpb improvements, and promote optimized models through the approval gate
