...
FHIR (Fast Healthcare Interoperability Resources) is the modern healthcare data interoperability standard developed by HL7 International. It defines a RESTful API and a set of over 150 resource types — including Patient, Encounter, Observation, MedicationRequest, and Condition — that represent discrete healthcare concepts using JSON and XML. FHIR R4 (v4.0.1), finalized in 2018, remains the most widely adopted version globally and carries normative status guaranteeing backward compatibility. As of 2026, over 70% of countries report active FHIR use, the CMS Prior Authorization Rule (CMS-0057-F) mandating FHIR-based APIs for US healthcare organizations took effect in January 2026, and the US Core Implementation Guide has reached v8.0.1 (USCDI v5) with v9.0.0 (USCDI v6) planned for May 2026. While FHIR R5 was published in 2023, major EHR vendors (Epic, Oracle Health) do not yet support it — R4 remains the production standard.
...
- **Regulatory compliance gap:** The CMS Interoperability and Prior Authorization Final Rule (CMS-0057-F) mandated FHIR-based APIs for prior authorization, patient access, and provider directory effective January 2026. Without FHIR support, the PMS cannot participate in these now-required data exchanges. The Da Vinci Prior Authorization Support (PAS) FHIR Implementation Guide (see experiment 48) further mandates standardized electronic PA submission by January 2027.
...
| Regulatory changes to FHIR requirements | Low — need to update conformance | Design facade to be version-aware; plan for FHIR R5/R6 support via versioned endpoints (`/fhir/r4/`, `/fhir/r5/`). Note: HL7 US Realm Steering Committee confirmed US Core will move to FHIR R6 (no timeline set); R4 remains the production standard through at least 2027. |
...
|---|---|---|---|
| `fhir.resources` | Python package | 8.x+ | FHIR R4B Pydantic models for validation and serialization (R5 is now default; use `fhir.resources.R4B` sub-package for R4 resources) |
| `fhir-core` | Python package | Latest | Core FHIR primitives (Pydantic v2 based, powers `fhir.resources`) |
...
| Aspect | FHIR (This Experiment) | MCP (Experiment 09) | Sanford Guide (Experiment 11) | FHIR Prior Auth (Experiment 48) | NextGen FHIR (Experiment 49) |
|---|---|---|---|---|---|
| **Primary purpose** | External healthcare system interoperability | Internal AI-to-PMS integration | Clinical decision support for antimicrobials | Standardized electronic prior authorization | EHR data import from referring providers |
| **Standard body** | HL7 International | Linux Foundation (AAIF) | Sanford Guide (proprietary) | HL7 Da Vinci | HL7 / NextGen Healthcare |
| **Protocol** | FHIR R4 REST API (HTTP/JSON) | JSON-RPC over Streamable HTTP | REST API (HTTPS) | FHIR R4 (CRD/DTR/PAS profiles) | FHIR R4 REST API |
| **Data direction** | Bidirectional (import + export) | Inbound (AI reads PMS data) | Inbound (PMS queries treatment data) | Outbound (PA submission to payers) | Inbound (clinical data from NextGen EHRs) |
| **Authorization** | SMART on FHIR (OAuth 2.0 + clinical scopes) | OAuth 2.1 | API key | SMART on FHIR + payer OAuth | OAuth 2.0 (NextGen API credentials) |
| **Complementarity** | Foundation layer for all FHIR-based experiments | MCP can expose FHIR resources as MCP tools | Data can be modeled as FHIR `MedicationKnowledge` | Builds directly on this experiment's FHIR Facade | Consumes FHIR R4 resources from NextGen systems |
...
**FHIR Experiment Family:** This experiment (16) provides the foundational FHIR R4 Facade. Experiments 48 (FHIR Prior Authorization) and 49 (NextGen FHIR API) build directly on top of it — experiment 48 implements the Da Vinci CRD/DTR/PAS profiles for electronic PA submission, while experiment 49 uses the FHIR client capabilities to import referral data from NextGen EHR systems. The HL7v2 LIS integration (experiment 17) handles legacy lab messaging and can bridge to FHIR via v2-to-FHIR translation.
...
- [HL7 FHIR R4 Specification](https://hl7.org/fhir/R4/) — Official FHIR R4 (v4.0.1) standard, resource types, and REST specification
- [FHIR Security Module](https://hl7.org/fhir/security.html) — Security labels, access control, encryption, and audit requirements
- [SMART App Launch v2.2.0](https://build.fhir.org/ig/HL7/smart-app-launch/app-launch.html) — OAuth 2.0 authorization framework for FHIR applications (current STU)
- [US Core Implementation Guide v8.0.1](https://hl7.org/fhir/us/core/) — Current US Core profiles (USCDI v5); v9.0.0 (USCDI v6) planned May 2026
- [FHIR v6.0.0 Ballot (R6 Preview)](https://build.fhir.org/) — Upcoming FHIR R6 specification and normative resource status

### Architecture & Implementation
- [FHIR Integration with FastAPI (DEV Community)](https://dev.to/wellallytech/fhir-integration-build-modern-healthcare-apps-using-python-and-fastapi-5cdf) — FastAPI FHIR server implementation guide with fhir.resources
- [fhir.resources on PyPI](https://pypi.org/project/fhir.resources/) — Python Pydantic v2-based models for FHIR R4B/R5 validation and serialization
- [fhir-core on GitHub](https://github.com/nazrulworld/fhir-core) — Pydantic v2 foundation powering fhir.resources
...
- [HIPAA-Compliant FHIR API with FastAPI (Medium)](https://medium.com/@petercovingtonmitchell/building-a-hipaa-compliant-fhir-api-with-fastapi-a-step-by-step-guide-f6d2897383ee) — Step-by-step HIPAA-compliant FHIR API guide
...
- [HIPAA-Compliant FHIR API Security (SCIMUS)](https://thescimus.com/blog/how-to-build-a-hipaa-compliant-fhir-api-security-best-practices/) — Security best practices for FHIR API implementations
- [FHIR and APIs: Building Secure Healthcare Systems (Censinet)](https://censinet.com/perspectives/fhir-apis-building-secure-healthcare-systems) — Enterprise security patterns for FHIR

### Ecosystem & Adoption
- [FHIR R4 vs R5: Choosing the Right Version (Health Samurai)](https://www.health-samurai.io/articles/fhir-r4-vs-fhir-r5-choosing-the-right-version-for-your-implementation) — Why R4 remains the production standard; R5 breaking changes and vendor support gaps
...
- [PRD: HL7v2 LIS Integration](17-PRD-HL7v2LIS-PMS-Integration.md) — Legacy lab messaging with FHIR bridge potential
...
- [PRD: FHIR Prior Authorization](48-PRD-FHIRPriorAuth-PMS-Integration.md) — Da Vinci CRD/DTR/PAS profiles built on this FHIR Facade
- [PRD: NextGen FHIR API](49-PRD-NextGenFHIRAPI-PMS-Integration.md) — FHIR R4 data import from NextGen EHR systems
- [HL7 FHIR R4 Specification](https://hl7.org/fhir/R4/) — Complete FHIR R4 specification
- [SMART on FHIR Documentation](https://docs.smarthealthit.org/) — SMART App Launch v2.2.0 authorization framework
- [US Core Implementation Guide](https://hl7.org/fhir/us/core/) — US Core profiles for FHIR R4
